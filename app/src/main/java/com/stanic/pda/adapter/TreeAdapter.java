...
import com.stanic.pda.util.DensityUtil;
import com.stanic.pda.util.TreeUtils;
...
public class TreeAdapter extends BaseAdapter {
    ...
    public TreeAdapter(Context mcontext, List<TreePoint> pointList, HashMap<String, TreePoint> pointMap) {
        ...
        if (!"".equals(keyword)) {
            for (TreePoint tempTreePoint : pointList) {
                if (tempTreePoint.getNNAME().contains(keyword)) {
                    if ("root".equals(tempTreePoint.getISLEAF())) {   //非叶子节点
...
//            if ("root".equals(tempPoint.getPARENTID())) {
//                count++;
//            } else {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if ("0".equals(tempPoint.getISLEAF())) {  //如果为父节点
            if (!tempPoint.isExpand()) {    //不展开显示加号
                ...
                holder.icon.setImageResource(R.drawable.shape_point);
            } else {                        //展开显示减号
                holder.icon.setVisibility(View.VISIBLE);
